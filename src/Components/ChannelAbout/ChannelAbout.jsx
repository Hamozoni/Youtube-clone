  ...
  return (
    ...
    >
      <div className={`${theme} about-container`}>
        <nav className={`${theme} about-nav`} onClick={() => setIsAboutChannelOpen(false)}>
          <h3 className="about-title">{staticData?.about}</h3>
          <CloseIcon />
        </nav>
        <section className={`${theme} channel-desc`}>
          <h4 className={`${theme} describtion`}>{chanelDetail?.description}</h4>
          <section className={`${theme} channel-links`}>
            <h3>{staticData?.links}</h3>
