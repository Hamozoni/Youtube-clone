...
const SearchRecorder = () => {
  const { setIsRecording, theme, lang, staticData } = useContext(statesContext);
  ...
  return (
    <section className={`b-g-t-${theme} search-recorder  close`} onClick={(e) => closeRecorder(e)}>
      <div className={`back-color-${theme} recorder-container`}>
        <header className="recorder-header">
          ...
          <h3 className={`t-color-${theme}-3 listing`}>{isRecordingStart ? staticData.listening : staticData.startRecord}</h3>
        </header>
        <div className="recorder-box">
          <div onClick={handleTogleRecording} className={isRecordingStart && !isRecordingEnd ? `active mike-container back-color-${theme}-1 ` : "mike-container"}>
        ...
        <div className="recorder-input-box">
          <input ref={recorderInput} className={`border-c-${theme}-1 recorder-inp`} type="text" value={transcript} />
        </div>
      </div>
    </section>
  );
