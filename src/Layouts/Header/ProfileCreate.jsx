...
const ProfileCreate = ({ setIsAcountModel }) => {
  ...
  return (
    <div className="prof-create">
      ...
      <div className={` t-color-${theme} back-hov-c-${theme}-2  user-img icon open-menu`} onClick={() => setIsAcountNavOpen(!isAcountNavOpen)}>
        <AccountCircleOutlinedIcon className={`${theme} user-img open-menu`} onClick={() => setIsAcountModel(true)} />
      </div>
    </div>
  );
};
